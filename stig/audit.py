...
def get_audits(args):
    ...
    data = {}
    sql = """SELECT p.*, a.exec_order, a.audit_name
        FROM profile AS p
...
def main():
    ...
    for audit in audits:
        ...
        audit_path = 'audits/' + str(audit['audit_name']) if audit['audit_name'] else ''
        if audit_path and os.path.isfile(audit_path) and os.stat(audit_path).st_size > 0:
