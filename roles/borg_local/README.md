...
| `borg_local__exclude_files__host_vars` / <br> `borg_local__exclude_files__group_vars` | The list of files or directories which should be excluded from the backup. Excludes act as filters within the included paths. Subkeys: <ul><li>`file`: Mandatory, string. The file or directory to be excluded.</li><li>`state`: Optional, string. Either `present` or `absent`. Defaults to `present`.</li></ul> | <ul><li>`'/root/.cache'`</li><li>`'*.svn*'`</li><li>`'*.git*'`</li><li>`'*.tmp'`</li><li>`'*.temp'`</li><li>`'*/cache/*'`</li><li>`'*/log/*'`</li></ul> |
