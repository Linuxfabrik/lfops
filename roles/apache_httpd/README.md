...
| `apache_httpd__packages__group_var` / `apache_httpd__packages__host_var` | List. List of dictionaries of packages to install, related to Apache, using the OS package manager. Possible options:<br> * `name`: Mandatory, string. The package name.<br> * `state`: Mandatory, string. State of the package, one of `present`, `absent`. | [Have a look](https://github.com/Linuxfabrik/lfops/blob/main/roles/apache_httpd/defaults/main.yml) |
...
| `conf_allow_override` | String. Will be set in the ``<Directory>`` directive of the vHost.<br>[Apache Directive](https://httpd.apache.org/docs/2.4/String/core.html#allowoverride) | * app: `'None'`<br> * localhost: `'None'` |
