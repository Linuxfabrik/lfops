...
* Stapling only does something for a certificate whose issuer runs an OCSP responder. Let's Encrypt, which the `acme_sh` role obtains certificates from, no longer publishes one, so its certificates carry no OCSP URI at all. With stapling on, mod_ssl logs `AH02218` and `AH02604` at error level once per certificate and vHost on every start and every reload, and staples nothing. Switch it on for a CA that still answers, or point `SSLStaplingForceURL` at a responder from a vHost of your own.
* Type: String.
* Default: `'off'`
