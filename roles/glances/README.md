...
On RHEL 8 / 9 (and clones) and on Debian / Ubuntu, glances is installed from the distribution package. On RHEL 10 and clones (Rocky / Alma 10), `glances` is not packaged in EPEL 10, so the role installs it into a dedicated Python venv at `/opt/python-venv/glances` via the [linuxfabrik.lfops.python_venv](https://github.com/Linuxfabrik/lfops/tree/main/roles/python_venv) role and exposes the `glances` binary in `/usr/local/bin`.
