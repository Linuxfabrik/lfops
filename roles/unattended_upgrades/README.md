...
[Unattended Upgrades](https://wiki.debian.org/UnattendedUpgrades) is the Debian / Ubuntu mechanism that periodically pulls and installs security updates without operator interaction. This role deactivates it on Debian-based systems by setting both `APT::Periodic::Update-Package-Lists` and `APT::Periodic::Unattended-Upgrade` to `0` in `/etc/apt/apt.conf.d/20auto-upgrades`. The `unattended-upgrades` package itself is left installed.
