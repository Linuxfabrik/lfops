...
| `php__modules__host_var` /<br> `php__modules__group_var` | List of additional PHP modules that should be installed via the standard package manager. Subkeys:<br> * `name`: Mandatory, string. Name of the module package.<br> * `state`: State of the module package. Possible options: `absent`, `present`. Modules are removed first and then added. | * `php-opcache` |
