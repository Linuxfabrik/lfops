...
| `keycloak__db_vendor` | String. Specifies the database server Keycloak is supposed to use. Changing this requires a full run with the `keycloak` tag, as `kc.sh build` needs to be re-executed. Possible options:<br> * mariadb<br> * mssql<br> * mysql<br> * oracle<br> * postgres | `mariadb` |
...
| `keycloak__hostname_backchannel_dynamic` | Bool. Enables dynamic resolving of backchannel URLs, including hostname, scheme, port and context path. Set to `true` if your application accesses Keycloak via a private network. If set to `true`, `keycloak__hostname` needs to be specified as a full URL. | `false` |
