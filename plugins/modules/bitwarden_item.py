...
def diff_and_update(current, target):
    ...
    def check_dict_for_changes(current, target):
        ...
        for key, value in target.items():
            ...
            elif (value != current.get(key)) and not (
                not value and not current.get(key)
            ):  # compare None to empty lists and empty strings
                changed = True
...
def run_module():
    ...
    module_args = dict(
        ...
        password=dict(
            type='str', required=False, default=None, no_log=False
        ),  # if we set it to True, the passwords in the RETURN values are masked too. see https://github.com/ansible/ansible/issues/71789
    ...
    if attachments:
        ...
        if len(set(basenames)) < len(basenames):
            module.fail_json(
                msg='This module cannot handle multiple attachments with the same basename.'
            )

        for attachment in attachments:
            if not os.access(attachment, os.R_OK):
                module.fail_json(
                    msg=f'Could not read the attachments at "{attachment}".'
                )
