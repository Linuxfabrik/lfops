...
def add_armored_exports_and_exit(gpg, module, result):
    ...
    if not ascii_armored_private_key:
        ...
        module.fail_json(
            msg='Failed to export armored private key. Is the passphrase correct?',
            **result,
        )
    result['ascii_armored_private_key'] = ascii_armored_private_key

    ascii_armored_public_key = gpg.export_keys(
        result['key']['fingerprint'], passphrase=module.params['passphrase']
    )
...
def run_module():
    ...
    try:
        ...
        module.fail_json(
            msg=f'There was an error executing gpg: {to_native(e)}',
    ...
    if keys.returncode != 0:
        module.fail_json(
            msg='Failed to list current keys.',
            rc=keys.returncode,
            stdout=keys.data,
            stderr=keys.stderr,
            **result,
    ...
    for key in keys:
        ...
        if matched:
            if match is None:
                ...
                module.fail_json(
                    msg='Found multiple keys with the same attributes, cannot decide which one to use. Aborting.',
                    **result,
                )
    ...
    if not new_key:
        ...
        module.fail_json(
            msg='Failed to generate a new key.',
            rc=new_key.returncode,
            stdout=new_key.data,
            stderr=new_key.stderr,
            **result,
        )
