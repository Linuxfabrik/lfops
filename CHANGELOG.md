...
* **role:wordpress, playbook:setup_wordpress**: The WordPress core, `wp-config.php` and `wp-content/mu-plugins` belong to `root`, so a vulnerable plugin can no longer plant code in them; only `wp-content` stays writable for the web server, and plugins and themes can still be installed from the web interface. WordPress no longer updates its core itself: `wordpress-core-minor-update-<instance>.timer` installs minor releases, and `--tags wordpress:update` everything else. After changing the permalink structure in the web interface, add the displayed rewrite rules to `.htaccess` by hand. `setup_wordpress` skips the document root chown of `apache_httpd`.
* **role:wordpress**: The REST API is restricted to logged-in users by the maintained Disable WP REST API plugin instead of Disable REST API (`disable-json-api`), which the role uninstalls. Unlike the old plugin, it also blocks the routes of plugins installed after its activation, so a front-end feature that calls the REST API without a login, such as a contact form, stops working until it gets an exception in code.
* **role:wordpress**: Application passwords are switched off, since they let REST API clients log in without a second factor. Set `wordpress__application_passwords_enabled: true` for sites whose integrations use them, such as the WordPress mobile app.
* **role:wordpress**: WordPress takes the client address from `X-Forwarded-For` only on requests from a proxy listed in `wordpress__trusted_proxies`, and ignores `X-Forwarded-Host`. Until now any client could set its own address or host name this way. Behind a reverse proxy, list its IP address, otherwise WordPress sees the proxy's address for every visitor.
* **role:wordpress**: `wordpress__url` includes the scheme, for example `https://wordpress.example.com`, and the role aborts while it does not; the vHost and the installation directory stay the same. The role sets the site address (`home` and `siteurl`) to it on every run, so that on an `https://` site the front-end login cookie carries the `Secure` flag, which it lacked on sites installed by the role. An address changed in the WordPress settings is set back, and HTTPS for the login and the admin area is only forced for an `https://` URL.
...
* **role:wordpress**: Several WordPress instances can share a host as pseudo hosts in the inventory. The cron timer, the core update timer, the vHost file and the export directory carry the host of `wordpress__url` in their name, for example `wordpress-cron-wordpress.example.com.timer`, and the role removes `wordpress-cron.timer` and the vHost file `wordpress.conf`. The README lists what the inventory needs.
...
* **role:wordpress**: `--tags wordpress:export` writes the export to `/backup/wordpress-export/<instance>`, which only `apache` and `root` can read, instead of to `/tmp`. The export holds unpublished content and the e-mail addresses of the users.
...
* **role:icingadb, role:icingaweb2, role:icingaweb2_module_reporting, role:icingaweb2_module_x509, role:mariadb_server**: The MariaDB tasks move from `ansible.mysql` to its MariaDB counterpart `ansible.mariadb`, so the deprecation warning printed on every run is gone and the roles keep working once `ansible.mysql` drops MariaDB support. `ansible-galaxy collection install linuxfabrik.lfops` pulls the new collection in as a dependency; a manually maintained collection list has to add it. If you run LFOps in the Execution Environment, pull the current image (`podman pull ghcr.io/linuxfabrik/lfops_ee:latest`), because an EE ships the collections it was built with and an older one aborts these roles with `couldn't resolve module/action`.
* **role:php**: The session cookie is marked `Secure`, so browsers only send it back over HTTPS. A site served over plain HTTP can no longer log anyone in, because the browser accepts the cookie and never returns it: set `php__ini_session_cookie_secure__group_var: 'Off'` for such a host, or `php_value_session_cookie_secure: 'Off'` for the affected pool alone. HTTPS hosts are unaffected, including behind a reverse proxy that terminates TLS, and nobody is logged out by the change: a running session resumes and keeps its existing cookie until the application regenerates the session ID.
* **role:php**: The session cookie carries `SameSite=Lax` from PHP 7.3 on, where PHP so far emitted no attribute at all and left the decision to the browser. The cookie is no longer sent on cross-site POSTs, in iframes or on XHR, which is what stops a foreign page from acting under a visitor's session. An application whose identity provider returns through a cross-site POST, as SAML HTTP-POST binding and the OIDC `form_post` response mode do, loops on login until it is set back, for that pool alone via the new `php_value_session_cookie_samesite: 'None'` and `php_value_session_cookie_secure: 'On'` subkeys of `php__fpm_pools__*_var` (browsers require `Secure` for `None`), or host-wide via `php__ini_session_cookie_samesite__group_var`. An identity provider under the same registrable domain is unaffected.
...
* **role:fail2ban**: The `apache-404-matomo` filter and jail work like `apache-404` but for the matomo LogFormat, where the virtual host precedes the client IP in the log line.
* **role:fail2ban**: The `apache-404` filter accepts `fail2ban__jail_apache_404_ignoreregex`, a list of regular expressions whose matching log lines are excluded from the 404 count, so known missing resources like `/favicon.ico` or `/assets/style.css` no longer trigger a ban.
...
* **role:librenms**: The session cookie is marked as secure on hosts whose `librenms__url` is an `https://` URL, so a browser only sends it over HTTPS. LibreNMS reports the missing flag as a failure in its own validation. Set `librenms__config_session_secure_cookie` to override.
...
* **role:monitoring_plugins, role:repo_monitoring_plugins**: Add SLES 15 and SLES 16 support, installing the Linuxfabrik Monitoring Plugins from the SUSE channel of `repo.linuxfabrik.ch` and applying the SUSE-specific package version lock ([#245](https://github.com/Linuxfabrik/lfops/issues/245)).
...
* **role:apache_httpd**: Bump the Core Rule Set to 4.27.0, and update the two reverse-proxy snippets in `EXAMPLES.md` to use `ProxyPass` instead of `RewriteRule`, which `%`-decodes the URI pattern and breaks WebDAV apps such as Nextcloud on rename. See the [blog post](https://www.linuxfabrik.ch/blog/nextcloud-rewriterules-vs-proxypass).
...
* **roles**: Set `become: false` on tasks delegated to localhost across the collection, so they no longer try to call `sudo` on the Ansible controller and fail with `sudo: a password is required` where passwordless sudo is not set up ([#242](https://github.com/Linuxfabrik/lfops/issues/242)).
