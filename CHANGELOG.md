...
* **role:numishare, role:orbeon_forms**: Deploy per-collection Numishare projects via `numishare__projects__*` (combined-var pattern, `git_url` + optional `git_version`/`git_update`, or `tarball_url` + optional `tarball_strip_components`) into `numishare__projects_dir` (default `/opt/numishare-projects`), rewriting each project's `exist-config.xml` to `numishare__exist_url` (project pipelines read it via the project-relative `../../exist-config.xml`, so the upstream `localhost:8080` default would otherwise 404 every collection page). The `orbeon_forms` role symlinks that directory to `WEB-INF/resources/numishare-projects` so Numishare's page-flow resolves the public `/numishare/<collection>/` pages through `oxf:/numishare-projects/<collection>/...`. Each project's bundled `ui/` is also exposed as a theme named after the project (symlink under `numishare__themes_dir`), so a collection whose `config.xml` sets `<orbeon_theme>` to the project name renders with the project's own look without a separate `numishare__themes__*` entry.
* **playbooks/setup_numishare**: New playbook that wires up the full Numishare stack (`apps`, `apache_solr`, `apache_tomcat`, `numishare`, `existdb`, `orbeon_forms`) in the correct order, including the Solr-on-Numishare specifics (`apache_solr__security_manager_enabled: false`, `apache_solr__dump_cores: ['numishare']`, OS-level dependencies via `apache_solr__apps__apps__dependent_var`). Orbeon runs under the `/orbeon` context; the reverse proxy that maps the clean public URLs onto it runs on a separate host and is not part of this playbook.
* **role:orbeon_forms**: New role to deploy [Orbeon Forms](https://www.orbeon.com/) Community Edition into an existing Tomcat for the Numishare stack. Extracts the WAR exploded into the `/orbeon` context (`orbeon_forms__home` defaults to `webapps/orbeon`); a reverse proxy in front (on a separate host, e.g. role `apache_httpd`) maps the clean public URLs (`/id/...`, `/themes/...`, `/numishare/...`, `/admin/`, `/`) onto the `/orbeon` backend and denies `/orbeon/fr` (Form Runner / Builder, unused by Numishare). Rewrites `log4j2.xml` to absolute log paths under `/var/log/tomcat` (Orbeon's relative `../logs/` resolves to `/usr/share/logs/` on RHEL 10 Tomcat and silently disables every FileAppender), deploys the context descriptor `Catalina/localhost/<webapp>.xml` with `allowLinking="true"`, wires Numishare's `apps/` symlink and XQJ libraries, ships a Numishare-branded favicon at the legacy `/ops/images/orbeon-icon-16.{ico,png}` path (Orbeon 2023.1 dropped the upstream file), and injects managed Numishare blocks into `properties-local.xml` and `web.xml` (epilogue theme, container auth, security-constraint for `/numishare/admin/*`, `/themes/*` default-servlet mapping, longer session timeout)
* **role:numishare**: New role for [Numishare](https://github.com/ewg118/numishare), the open-source numismatic-collection platform. Clones the repository pinned to the mandatory `numishare__git_version` (e.g. the release tag `v1.2`), installs `fontconfig` + a font (`numishare__captcha_font_packages`) so Orbeon's on-premise CAPTCHA renders on the headless JVM (otherwise the feedback form breaks), deploys `exist-config.xml`, wires the Solr `numishare` core (notifies a Solr restart so the core is loaded), exposes Numishare's bundled UI as the `default` theme via a `/opt/themes/default` symlink, and supports custom themes via `numishare__themes__*` (combined-var pattern, source can be `git_url` + optional `git_version`/`git_update`, or `tarball_url` + optional `tarball_strip_components`)
