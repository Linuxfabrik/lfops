...
* **role:example**: The config-validation handler of the reference role now triggers the restart handler it notifies. Only the template new roles are copied from was affected, no role that manages an application.
...
* **role:php**: The `php:update` tag now also deploys the PHP-FPM pools and enables and starts the FPM service. Both live under version-specific paths on Debian (`/etc/php/<version>/fpm/pool.d`, `php<version>-fpm.service`), so without this a host that changed `php__version` would end up running the new version with none of its pools and a unit that was never enabled. On RedHat the paths do not change, so the tag simply reasserts what is already in place.
...
* **role:mastodon**: Fix the wrong dependent variable name for the Apache modules, so modules set via `apache_httpd__mods__host_var` in the inventory are no longer discarded. Rename `mastodon__apache_httpd__mods__host_var` to `__dependent_var` if you override it.
* **role:postgresql_server**: Fix the `grant_option` subkey of `postgresql_server__privs`, which never took effect. Also documents the entry's `roles` and `state` subkeys.
* **playbook:php, playbook:redis, playbook:setup_grav, playbook:setup_moodle**: These playbooks install packages from Remi's repository but never enabled EPEL (and CRB on Rocky 9 and newer), which Remi's packages depend on. On RedHat 8 this aborted the run with `nothing provides libcapstone.so.4 needed by php-opcache`, because neither the default repositories nor PowerTools ship it. The playbooks now enable both beforehand, skippable via `<prefix>__skip_repo_epel` and `<prefix>__skip_repo_baseos`.
...
* **role:apache_httpd, role:apache_solr, role:apache_tomcat, role:bind, role:blocky, role:chrony, role:clamav, role:collabora, role:coturn, role:docker, role:elasticsearch, role:fail2ban, role:glpi_agent, role:grafana, role:graylog_datanode, role:graylog_server, role:icinga2_master, role:icinga_kubernetes, role:icingadb, role:influxdb, role:keepalived, role:kibana, role:logstash, role:mariadb_server, role:mongodb, role:opensearch, role:php, role:postfix, role:postgresql_server, role:redis, role:rocketchat, role:rsyslog, role:snmp, role:squid, role:sshd, role:systemd_journald, role:telegraf, role:vsftpd**: A configuration change no longer restarts or reloads a service that the very same run had just started, so a fresh installation no longer bounces the service right after starting it. A configuration change also no longer starts a service that is configured to stay stopped.
* **role:apache_tomcat**: `apache_tomcat__service_state` now takes effect. It was documented and defaulted to `started`, but the role ignored it and always derived the service state from `apache_tomcat__service_enabled`. Its default is now `started` if `apache_tomcat__service_enabled` is `true` and `stopped` otherwise, so hosts that never set it are unaffected. Hosts that set `apache_tomcat__service_state` to a value contradicting `apache_tomcat__service_enabled` now get the state they asked for instead of having the variable silently ignored.
